...
import { ChatCompletion, ChatCompletionCreateParamsNonStreaming } from "@cerebras/cerebras_cloud_sdk/resources/chat/completions";
...
export class CerebrasInstrumentation extends InstrumentationBase {
    ...
    try {
      ...
      if (this._shouldSendPrompts()) {
        if (type === "chat") {
          ...
          attributes[`${SpanAttributes.LLM_PROMPTS}.0.content`] = params.prompt as string;
        }
      }
    } catch (e) {
      this._diag.debug(e);
  ...
  private async *_streamingWrapPromise({
    ...
    if (type === "chat") {
      ...
      for await (const chunk of await promise) {
        ...
        try {
          message.content += (chunk as ChatCompletion.ChatChunkResponse).choices?.[0]?.delta?.content || "";
        } catch (e) {
          this._diag.debug(e);
          this._config.exceptionLogger?.(e);
        }
      ...
      for await (const chunk of await promise) {
        ...
        try {
          result.choices[0].text += (chunk as Completion.CompletionResponse).choices?.[0]?.text || "";
        } catch (e) {
          this._diag.debug(e);
          this._config.exceptionLogger?.(e);
        }
  ...
  private _endSpan({
    ...
    | { span: Span; type: "chat"; result: ChatCompletion.ChatCompletionResponse }
    | {
        span: Span;
        type: "completion";
    ...
    try {
      ...
      if (type === "chat" && result.usage) {
        span.setAttribute(
          ...
          (result.usage.input_tokens as number | undefined ?? 0) + (result.usage.output_tokens as number | undefined ?? 0),
        );
        span.setAttribute(
          SpanAttributes.LLM_USAGE_COMPLETION_TOKENS,
          (result.usage.output_tokens as number | undefined ?? 0),
        );
        span.setAttribute(
          SpanAttributes.LLM_USAGE_PROMPT_TOKENS,
          (result.usage.input_tokens as number | undefined ?? 0),
        );
      }
